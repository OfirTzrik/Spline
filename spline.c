...
void drawSplineSegment(struct Circle* circles, int* circlesArraySize) {
  ...
  for(int i = 0; i < *circlesArraySize - 3; i++) {
    ...
    for(float t = t1; t <= t2; t += SPLINE_SAMPLE_RATE) {
      a1.center.x = ((t1-t)/(t1-t0))*p0.center.x + ((t-t0)/(t1-t0))*p1.center.x;
      a1.center.y = ((t1-t)/(t1-t0))*p0.center.y + ((t-t0)/(t1-t0))*p1.center.y;
      
      a2.center.x = ((t2-t)/(t2-t1))*p1.center.x + ((t-t1)/(t2-t1))*p2.center.x;
      a2.center.y = ((t2-t)/(t2-t1))*p1.center.y + ((t-t1)/(t2-t1))*p2.center.y;
      
      a3.center.x = ((t3-t)/(t3-t2))*p2.center.x + ((t-t2)/(t3-t2))*p3.center.x;
      a3.center.y = ((t3-t)/(t3-t2))*p2.center.y + ((t-t2)/(t3-t2))*p3.center.y;
      
      b1.center.x = ((t2-t)/(t2-t0))*a1.center.x + ((t-t0)/(t2-t0))*a2.center.x;
      b1.center.y = ((t2-t)/(t2-t0))*a1.center.y + ((t-t0)/(t2-t0))*a2.center.y;
      
      b2.center.x = ((t3-t)/(t3-t1))*a2.center.x + ((t-t1)/(t3-t1))*a3.center.x;
      b2.center.y = ((t3-t)/(t3-t1))*a2.center.y + ((t-t1)/(t3-t1))*a3.center.y;
      
      q.center.x = ((t2-t)/(t2-t1))*b1.center.x + ((t-t1)/(t2-t1))*b2.center.x;
      q.center.y = ((t2-t)/(t2-t1))*b1.center.y + ((t-t1)/(t2-t1))*b2.center.y;
